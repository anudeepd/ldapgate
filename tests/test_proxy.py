...
def test_backend_error_generic_message(client, monkeypatch):
...
def test_options_request_requires_auth(client, monkeypatch):
...
def test_logout_rejects_cross_origin():
    ...
    with TestClient(app, follow_redirects=False) as tc:
        ...
        resp = tc.post("/_auth/logout", headers={
...
def test_login_router_post_works():
    ...
    with TestClient(app, follow_redirects=False) as tc:
        with patch("ldapgate.proxy.LDAPAuthenticator.authenticate", return_value=True), \
