...
from ldapgate.config import LDAPSettings, ProxySettings, load_config
...
def test_ldap_settings_invalid_tls_validate():
    ...
    with pytest.raises(ValueError, match='must be NONE, OPTIONAL, or REQUIRED'):
        LDAPSettings(
            url='ldaps://dc.example.com:636',
            bind_dn='CN=svc,CN=Users,DC=example,DC=com',
            bind_password='secret',
...
def test_backend_url_blocks_non_local_http():
    ...
    with pytest.raises(ValueError, match='Backend URL must use https://'):
        ProxySettings(
...
def test_cookie_samesite_validation():
    ...
    with pytest.raises(ValueError, match='cookie_samesite must be'):
        ProxySettings(
            backend_url='http://localhost:8080',
            secret_key='x9Q#mK2vL$pN4wR8tJ6bY3cH7fG1eA5!',
...
def test_session_cookie_name_validation():
    ...
    with pytest.raises(ValueError, match='must be a valid cookie name'):
        ProxySettings(
            backend_url='http://localhost:8080',
            secret_key='x9Q#mK2vL$pN4wR8tJ6bY3cH7fG1eA5!',
    ...
    with pytest.raises(ValueError, match='must not include the __Host- prefix'):
        ProxySettings(
            backend_url='http://localhost:8080',
            secret_key='x9Q#mK2vL$pN4wR8tJ6bY3cH7fG1eA5!',
