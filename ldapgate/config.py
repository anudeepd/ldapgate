...
def validate_backend_url(url: str) -> str:
    ...
    if v_lower.startswith("http://"):
        if not re.match(r"^http://(localhost|127\.0\.0\.1|\[::1\])(:\d+)?(/|$)", v_lower):
            raise ValueError(
                "Backend URL must use https:// for non-localhost addresses. "
                "Plain HTTP over the network is insecure."
            )
    return url
...
class LDAPSettings(BaseModel):
    ...
    require_authorization_rule: bool = Field(
        True,
        description="Require allowed_users or group_dn to be configured. "
                    "Disable only for local development or tests.",
    )
    ...
    follow_referrals: bool = Field(
        False, description="Automatically follow LDAP referrals (default: False)"
    )
    ...
    @classmethod
    def _validate_allowed_users(cls, v: Optional[List[str]]) -> Optional[List[str]]:
        if v is None:
            return v
...
class ProxySettings(BaseModel):
    ...
    revocation_path: Optional[str] = Field(
        None,
    ...
    rate_limit_state_path: Optional[str] = Field(
        None,
    ...
    @classmethod
    def _validate_backend_url(cls, v: Optional[str]) -> Optional[str]:
        if v is None:
            return v
    ...
    def _validate_auth_paths(cls, v: str) -> str:
        ...
        if not v.startswith("/_auth/"):
            ...
            _logging.getLogger(__name__).warning(
                "Auth path '%s' is not under '/_auth/'. Ensure it does not "
                "overlap with backend application routes.", v
            )
        return v
